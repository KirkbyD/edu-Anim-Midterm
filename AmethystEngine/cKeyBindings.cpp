			...
			/*	if ((*itModelObjects).second->velocity.x > maxSpeed)
				...
				if ((g_vec_pComplexObjects[vpComplexObjectsCurrentPos]->HasAnimations())) {
					...
					if (animaName != "Jump" && animaName != "Fall" && animaName != "Death") {
						...
						if (key == GLFW_KEY_S) {
							if (animaName == "WalkF" || animaName == "WalkB" || animaName == "RunF" || animaName == "RunB")
								g_vec_pComplexObjects[vpComplexObjectsCurrentPos]->QueueAnimation("Roll");
						}
						if (key == GLFW_KEY_SPACE) {
							g_vec_pComplexObjects[vpComplexObjectsCurrentPos]->QueueAnimation("Jump");
						}
