	...
	if (AnimaQueue.size() > 1) {
		...
		if (AnimaQueue.front() == "Idle" && AnimaName != "Idle") {
			...
			if (mapAnimationFriendlyNameTo_pScene.find(AnimaName) != mapAnimationFriendlyNameTo_pScene.end()) {
				...
				cComplexObject* pParent = dynamic_cast<cComplexObject*>(this->GetParent());

				if (AnimaName == "WalkF") {
					pParent->SetOrientation(glm::quatLookAt(glm::vec3(0.f, 0.f, -1.f), glm::vec3(0.f, 1.f, 0.f)));
				}
				else if (AnimaName == "WalkB") {
					pParent->SetOrientation(glm::quatLookAt(glm::vec3(0.f, 0.f, 1.f), glm::vec3(0.f, 1.f, 0.f)));
				}
				else if (AnimaName == "RunF") {
					pParent->SetOrientation(glm::quatLookAt(glm::vec3(0.f, 0.f, -1.f), glm::vec3(0.f, 1.f, 0.f)));
				}
				else if (AnimaName == "RunB") {
					pParent->SetOrientation(glm::quatLookAt(glm::vec3(0.f, 0.f, 1.f), glm::vec3(0.f, 1.f, 0.f)));
				}

				AnimaTime = 0;
				prevAnimName = AnimaName;
				return true;
			}
		}
